...
points <- superimpose(points, pp_cluster)
radius2 <- 20/160
pp_cluster <- circle(distribution=cluster_type, n=10*floor(density_c*radius2^2*pi), radius=radius2, centre=c(3,3), sig=radius2/4)
points <- superimpose(points, pp_cluster)
fileName = 'test5'
plot(points, cex=0.2, main='', pch=16, cols="black", use.marks=FALSE, cex.lab = 1.5, cex.axis = 1.5, cex.main = 1.5)
openpdf(paste(fileName, ".pdf", sep = ''))
...
# area <- 20*20  # in STORM pixels
# num_clusters <- 10
# rcluster <- c(50/160, 100/160)  # mean/s of radius/radii. Univariate or bivariate distribution 
# scluster <- c(0.05, 0.1)
# density_c <- 100  # constant
# enrichment <- 50
# density_b <- density_c/enrichment
# nbackground <- floor(area*density_b)
# 
...
#                     round(160*rcluster[1]), '_', round(160*rcluster[2]), '_enrich', enrichment, '_densityc', 
#                     round(density_c*160^2)), collapse='')
# openpdf(paste(fileName, ".pdf", sep = ''))
# plot(points, cex=0.2, main='', pch=16, cols="black", use.marks=FALSE, cex.lab = 1.5, cex.axis = 1.5, cex.main = 1.5)
# closepdf(paste(fileName, ".pdf", sep = ''))
# savetext(matrix(c(points$x,points$y), nrow=points$n, ncol=2), paste(fileName, ".txt", sep = ''))
